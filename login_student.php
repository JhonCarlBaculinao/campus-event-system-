...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (rmc_rate_is_blocked($conn, $rl_key, $rl_max, $rl_window)) {
        ...
        $error = 'Student access is currently disabled by the administrator. Please try again later.';
    } else {
        ...
        if ($query === false) {
            ...
            if (!$user) {
                ...
                if ($actual_role !== null && $actual_role !== $selected_role) {
                    ...
                    $error = sprintf(t('invalid_credentials'), 'Student');
                }
        ...
        <div class="bg-white/90 backdrop-blur-xl shadow-2xl rounded-[26px] border border-slate-200/60 w-full max-w-md p-8 sm:p-10 animate-up relative overflow-hidden">
            ...
            <form method="POST" action="" class="mt-6 space-y-6" autocomplete="on">
                ...
                <div>
                    ...
                    <input type="text" name="student_id" id="idInput" required autocomplete="username" maxlength="100" placeholder="<?= t('placeholder_student_id'); ?>" class="mt-2 w-full rounded-xl border border-slate-200 px-5 py-3 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-rmc-200 focus:border-rmc-300 outline-none transition">
                </div>
