...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (rmc_rate_is_blocked($conn, $rl_key, $rl_max, $rl_window)) {
        ...
        $error = 'Organizer access is currently disabled by the administrator. Please try again later.';
    } else {
        ...
        if ($query === false) {
            ...
            if (!$user) {
                ...
                if ($actual_role !== null && $actual_role !== $selected_role) {
                    ...
                    $error = sprintf(t('invalid_credentials'), 'Organizer');
                }
            } elseif (strtolower($user['status']) === 'pending') {
                $error = 'Your organizer application is still pending admin approval. Please check back later.';
            } elseif (strtolower($user['status']) === 'rejected') {
                $error = 'Your organizer application was not approved. Please contact the administrator for details.';
            ...
            <form method="POST" action="" class="mt-6 space-y-6" autocomplete="on">
                ...
                <div>
                    <label for="idInput" class="font-semibold text-slate-700">
                        <?= t('organizer_username'); ?>
                    </label>
                    <input type="text" name="student_id" id="idInput" required autocomplete="username" maxlength="100" placeholder="<?= t('placeholder_organizer_username'); ?>" class="mt-2 w-full rounded-xl border border-slate-200 px-5 py-3 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-rmc-200 focus:border-rmc-300 outline-none transition">
                </div>
...
<a href="login_admin.php" class="inline-flex items-center gap-2 bg-white hover:bg-rmc-50 border border-rmc-200 text-rmc-800 px-4 py-2.5 rounded-xl text-sm font-semibold transition"><i class="fa-solid fa-shield-halved"></i> Admin Login</a>
                </div>
            </div>
